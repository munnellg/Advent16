...
static int g_target_lo, g_target_hi;
void
...
update( int bot_id ) {
	...
	if( bot->lo && bot->hi ) {
		if(bot->lo == g_target_lo && bot->hi == g_target_hi) {
			printf("Bot %d compared value-%d and value-%d chips\n", bot_id, bot->lo, bot->hi);
		}
...
main( int argc, char *argv[] ) {
	...
	if( argc < 4 ) {
		printf("Usage: ./d10p1 FILE NUM NUM\n");
		return EXIT_FAILURE;
	}
	...
	g_target_hi = atoi(argv[2]);
	g_target_lo = atoi(argv[3]);

	if( g_target_hi < g_target_lo) {
		g_target_hi ^= g_target_lo;
